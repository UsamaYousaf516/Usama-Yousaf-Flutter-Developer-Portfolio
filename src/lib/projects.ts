...
export const projects: Project[] = [
  ...
  },
  {
    ...
    isFeatured: true,
    shortDescription:
      "An algorithm battle-royale where 20 named AI strategies fight for grid territory, packaged as a vertical-video content studio for Shorts, Reels, and TikTok.",
    longDescription:
      "Pixel Clash (\"Territory Studio\") is a self-directed creative-technology build: 20 named algorithmic \"Patterns\" — each a distinct strategy like Hunter (chases the current leader), Mirror (copies whatever's winning), or Guardian (turtles up and outlasts) — compete across 38 procedurally varied arenas for grid territory. Every battle is deterministic from a single seed, replayable and checksum-verified, and exportable straight from the browser as a vertical (9:16) clip ready for short-form video — no manual recording, no server-side rendering.",
    role: "Solo creative technologist — simulation engine, backend, data model, deployment",
    team: "Solo project",
    ...
    challenge:
      "The goal was an algorithm battle-royale that's genuinely watchable as short-form vertical video — legible territory battles between very differently-behaved AI strategies — without any manual video editing, and with every battle exactly reproducible for debugging and verification. That reproducibility requirement (same seed, same battle, bit-for-bit) had to be designed in from the start rather than bolted on later.",
    responsibilities: [
      "Designed 20 named algorithmic Patterns (random walk, snake, hunter, mirror, comeback, solidify, ambush, burst, flood, centerpull, and more), each with its own stat profile and flavor writing",
      "Built 38 procedurally varied arenas across multiple generation modes (scatter, maze, and others) with difficulty and special-rule variation",
      "Implemented a deterministic, seeded simulation engine (custom xorshift PRNG) so any battle can be exactly replayed from its seed",
      "Built a custom Node.js HTTP server — no framework — with Zod-validated REST endpoints and a Prisma/SQLite persistence layer",
      "Implemented client-side vertical (9:16) video capture via canvas.captureStream + MediaRecorder, with no server-side rendering",
      "Built an automatic highlight generator that derives commentary (rival lead time, comebacks, first-arena wins, win streaks) directly from raw simulation data",
      "Built a leaderboard and analytics dashboard (composite ranking score, win rate, average territory, streaks) from battle history",
    ],
    features: [
      {
        name: "20 algorithmic Patterns",
        benefit: "Each fighter reads as a distinct strategy — Hunter chases the leader, Mirror copies whoever's winning, Guardian turtles up — not random movement",
        technicalNote: "Each Pattern pairs a named algorithm archetype (hunter, mirror, solidify, flood, centerpull, etc.) with its own speed/aggression/defense/chaos stat profile",
      },
      {
        name: "Deterministic seeded replay",
        benefit: "Any interesting battle can be perfectly reproduced and verified, not just re-approximated",
        technicalNote: "A custom xorshift PRNG seeds the entire simulation; seed, event log, and a replay checksum are persisted so the exact match can be regenerated",
      },
      {
        name: "Vertical video capture",
        benefit: "Battles export as ready-to-post 9:16 clips for Shorts/Reels/TikTok with zero manual editing",
        technicalNote: "canvas.captureStream() feeds a MediaRecorder session directly in the browser — no server-side rendering pipeline involved",
      },
      {
        name: "Auto-generated highlights",
        benefit: "Every battle gets sportscaster-style commentary without a human writing it",
        technicalNote: "Highlight text is derived from the raw lead-timeline and battle history: rival lead duration, comeback rank, first-arena win, and streak detection",
      },
    ],
    engineeringDecisions: [
      "Wrote a plain Node.js HTTP server instead of reaching for a framework, since the whole API surface is a handful of small JSON endpoints — kept the deploy footprint minimal",
      "Chose a deterministic, seeded PRNG over Math.random() specifically so battles are exactly reproducible — this is what makes the replay/checksum system possible and made simulation bugs far easier to track down",
      "Used the browser's own canvas.captureStream + MediaRecorder for video export instead of a server-side rendering pipeline, trading some encoding control for zero added infrastructure",
      "Modeled a richer relational schema (Pattern/Arena/Battle/BattleReplay/Video) in Prisma alongside the simpler flat battle table the MVP writes to today — scaffolded for the next iteration without blocking the shipped build",
    ],
    outcomes: [
      "Shipped a working build with 20 distinct AI Patterns and 38 arenas, deployed on Render with a persistent SQLite disk",
      "Built a fully deterministic, replayable simulation — any battle can be regenerated bit-for-bit from its seed",
      "Delivered a zero-infrastructure vertical video export pipeline — browser-native capture, no server-side rendering",
      "Built automatic highlight generation and a composite leaderboard ranking, so the dashboard needs no manual curation",
    ],
    lessons: {
      learned:
        "Designing for reproducibility (seeded RNG) from day one made the replay and highlight-generation systems dramatically simpler than retrofitting determinism later would have been.",
      improve:
        "Would move the relational Pattern/Arena/Battle schema to the primary write path sooner, instead of running it alongside the simpler flat battle table.",
      explore:
        "Interested in migrating the UI onto the Next.js/React/Zustand layer already scaffolded alongside this, and in exploring server-assisted rendering for longer highlight reels beyond a single battle.",
    },
    images: [],
    videos: [],
  },
